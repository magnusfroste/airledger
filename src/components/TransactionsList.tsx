...
const TransactionsList = () => {
  ...
  const fetchTransactions = async () => {
    try {
      ...
      } = await supabase.from('airledger_transactions').select('*').order('transaction_date', {
        ascending: false
      });
      if (transactionsError) throw transactionsError;

      // Fetch entries for each transaction
      const transactionsWithEntries = await Promise.all((transactionsData || []).map(async transaction => {
        const {
          data: entries,
          error: entriesError
        } = await supabase.from('airledger_entries').select('*').eq('transaction_id', transaction.id).order('account_code');
        if (entriesError) {
  ...
  const filteredTransactions = transactions.filter(transaction => {
    const matchesSearch = transaction.description.toLowerCase().includes(searchTerm.toLowerCase()) || transaction.analysis_data?.vendor && transaction.analysis_data.vendor.toLowerCase().includes(searchTerm.toLowerCase());
  ...
  const deleteTransaction = async (transactionId: string) => {
    try {
      ...
      const {
        error: entriesError
      } = await supabase.from('airledger_entries').delete().eq('transaction_id', transactionId);
      if (entriesError) throw entriesError;

      // Then delete the transaction itself
      const {
        error: transactionError
      } = await supabase.from('airledger_transactions').delete().eq('id', transactionId);
  ...
  const handleExportCSV = () => {
    ...
    transactions.forEach(transaction => {
      transaction.entries.forEach(entry => {
        csvRows.push([transaction.transaction_date, `"${transaction.description}"`, transaction.reference_number || '', transaction.transaction_type, entry.account_code, `"${entry.account_name}"`, entry.debit_amount || 0, entry.credit_amount || 0, `"${entry.description || ''}"`].join(','));
      });
    });
  ...
  const parseCSVForPreview = async (csvText: string) => {
    ...
    const {
      data: accountsData
    } = await supabase.from('airledger_chart_of_accounts').select('account_code, account_name').eq('is_active', true);
    ...
    try {
      ...
      for (const [key, group] of transactionGroups) {
        try {
          ...
          const totalAmount = group.entries.reduce((sum: number, entry: any) => sum + Math.max(entry.debit_amount, entry.credit_amount), 0);

          // Create transaction
          const {
            data: transactionData,
            error: transactionError
          } = await supabase.from('airledger_transactions').insert({
            ...group.transaction,
          ...
          const {
            error: entriesError
          } = await supabase.from('airledger_entries').insert(entriesWithTransactionId);
          if (entriesError) throw entriesError;
          successCount++;
        } catch (error) {
  ...
  return <div className="min-h-screen bg-gray-50">
        ...
        <div className="max-w-6xl mx-auto px-4 md:px-6 py-4 md:py-8">
          <div className={`flex items-center justify-between ${isMobile ? 'flex-col space-y-4' : ''}`}>
            ...
            <div className={`flex items-center ${isMobile ? 'flex-row space-x-2 w-full justify-end' : 'space-x-3'}`}>
              {/* Export Button */}
              <Button onClick={handleExportCSV} variant="outline" size="sm" className="p-2" disabled={transactions.length === 0} title="Exportera transaktioner som CSV">
              ...
              <Dialog open={importDialogOpen} onOpenChange={setImportDialogOpen}>
                ...
                <DialogContent className="sm:max-w-6xl">
                  ...
                  <div className="space-y-4">
                    ...
                    {importPreview.length > 0 && <div className="space-y-4">
                        ...
                        <div className="max-h-80 overflow-y-auto border rounded-lg">
                          <table className="w-full text-sm">
                            ...
                            <tbody>
                              {importPreview.map((row, index) => <tr key={index} className={`border-b ${!row.valid ? 'bg-red-50' : ''}`}>
                                  ...
                                  <td className="p-2">
                                    {row.valid ? <Badge variant="default" className="text-xs">Giltig</Badge> : <div>
                    ...
                    <div className="flex gap-2 pt-4">
                      <Button onClick={handleImportCSV} disabled={importLoading || importPreview.filter(r => r.valid).length === 0} className="flex-1">
        ...
        <div className="bg-white rounded-xl border border-gray-200 p-4">
          <div className="flex items-center space-x-4">
            <div className="relative flex-1">
              ...
              <Input placeholder="Sök transaktioner..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} className="pl-10 border-gray-200 focus:border-blue-500 focus:ring-blue-500" />
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="max-w-6xl mx-auto md:px-6 pb-8 px-0">
        <div className="space-y-4">
          {filteredTransactions.map(transaction => <Card key={transaction.id} className="bg-white border border-gray-200 hover:shadow-md transition-shadow">
              <CardContent className="p-0">
                <div className={`cursor-pointer ${isMobile ? 'p-4' : 'p-6'}`} onClick={() => setExpandedTransaction(expandedTransaction === transaction.id ? null : transaction.id)}>
                  <div className={`flex items-center justify-between ${isMobile ? 'flex-col space-y-3' : ''}`}>
                     <div className={`flex items-center ${isMobile ? 'w-full' : 'space-x-4'}`}>
                       ...
                       <div className="flex-1">
                         <div className={`flex items-center gap-2 ${isMobile ? 'justify-between' : ''}`}>
                           ...
                           {(transaction.image_metadata || transaction.image_url) && <ReceiptThumbnail imagePath={transaction.image_metadata?.storagePath || transaction.image_url} thumbnailPath={transaction.image_metadata?.thumbnailPath} metadata={transaction.image_metadata} analysis={transaction.analysis_data} compact={true} showActions={false} />}
                         </div>
                         <div className={`flex items-center mt-1 ${isMobile ? 'justify-between' : 'space-x-4'}`}>
                           <div className="flex items-center space-x-2">
                             ...
                             {transaction.analysis_data?.vendor && !isMobile && <span className="text-sm text-gray-500">• {transaction.analysis_data.vendor}</span>}
                           </div>
                           {isMobile && <span className={`text-lg font-semibold ${transaction.transaction_type === 'income' ? 'text-green-600' : 'text-gray-900'}`}>
                               {transaction.transaction_type === 'income' ? '+' : ''}
                               {formatAmount(transaction.total_amount)}
                             </span>}
                         </div>
                         {isMobile && transaction.analysis_data?.vendor && <div className="mt-1">
                     ...
                     <div className={`flex items-center ${isMobile ? 'w-full justify-between' : 'space-x-4'}`}>
                       ...
                       <div className="flex items-center space-x-2">
                         <Button variant="ghost" size={isMobile ? "sm" : "sm"} onClick={e => {
                      e.stopPropagation();
                      ...
                      deleteTransaction(transaction.id);
                         ...
                         <ChevronRight className={`h-5 w-5 text-gray-400 transition-transform ${expandedTransaction === transaction.id ? 'rotate-90' : ''}`} />
                       </div>
                     </div>
                  </div>
                </div>
                ...
                {expandedTransaction === transaction.id && transaction.entries.length > 0 && <div className="border-t border-gray-100 bg-gray-50">
                    <div className={isMobile ? 'p-4' : 'p-6'}>
                      {/* Image thumbnail if available */}
                      {(transaction.image_metadata || transaction.image_url) && <div className={isMobile ? 'mb-4' : 'mb-6'}>
                          <h4 className={`font-medium text-gray-900 mb-3 ${isMobile ? 'text-sm' : ''}`}>Bifogad bild</h4>
                          <div className="w-fit">
                            <ReceiptThumbnail imagePath={transaction.image_metadata?.storagePath || transaction.image_url} thumbnailPath={transaction.image_metadata?.thumbnailPath} metadata={transaction.image_metadata} analysis={transaction.analysis_data} compact={isMobile} showActions={!isMobile} />
                          </div>
                        </div>}
                      
                      <h4 className={`font-medium text-gray-900 mb-4 ${isMobile ? 'text-sm' : ''}`}>Kontering</h4>
                      <div className={isMobile ? 'space-y-2' : 'space-y-3'}>
                        {transaction.entries.map(entry => <div key={entry.id} className={`bg-white rounded-lg border border-gray-200 ${isMobile ? 'p-3' : 'py-3 px-4'}`}>
                            <div className={`flex ${isMobile ? 'flex-col space-y-2' : 'items-center justify-between'}`}>
                              <div className={`flex items-center ${isMobile ? 'space-x-2' : 'space-x-4'}`}>
                                <div className={`bg-blue-100 rounded-md flex items-center justify-center ${isMobile ? 'w-10 h-6' : 'w-12 h-8'}`}>
                                ...
                                <div className="flex-1">
                                  ...
                                  {entry.description && <p className={`text-gray-500 ${isMobile ? 'text-xs' : 'text-sm'}`}>
                                      {entry.description}
                                    </p>}
                                </div>
                              </div>
                              ...
                              <div className={`${isMobile ? 'flex justify-between text-sm' : 'text-right'}`}>
                                {entry.debit_amount > 0 && <div className={isMobile ? 'text-sm' : 'text-sm'}>
                                ...
                                {entry.credit_amount > 0 && <div className={isMobile ? 'text-sm' : 'text-sm'}>
                      ...
                      <div className={`mt-4 p-4 bg-blue-50 rounded-lg ${isMobile ? 'text-sm' : ''}`}>
                        ...
                        <div className="border-t border-blue-200 mt-2 pt-2 flex justify-between text-sm font-medium">
                          <span>Balans:</span>
                          <span className={Math.abs(transaction.entries.reduce((sum, entry) => sum + entry.debit_amount - entry.credit_amount, 0)) < 0.01 ? "text-green-600" : "text-red-600"}>
                            {Math.abs(transaction.entries.reduce((sum, entry) => sum + entry.debit_amount - entry.credit_amount, 0)) < 0.01 ? "✓ Balanserad" : "⚠ Obalanserad"}
                          </span>
                        </div>
                      </div>
                    </div>
      ...
      {/* Edit Dialog */}
      <TransactionEditDialog open={!!editingTransaction} onOpenChange={open => !open && setEditingTransaction(null)} transaction={editingTransaction!} onTransactionUpdated={fetchTransactions} />
    </div>;
};
